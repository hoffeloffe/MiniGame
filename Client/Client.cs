...
namespace NotAGame
{
    public class Client
    {
        ...
        private string serverip = "192.168.1.166";
        ...
        public void SendData()
        {
            ...
            while (true)
            {
                if (direct != null || direct != "" || cq.Count == 0)
                {
                    try
                    {
                        ...
                        string directy = direct;
                        if (cq.Count != 0)
                        {
                            cq.TryDequeue(out directy);
                        }
                        Byte[] sendBytes = Encoding.ASCII.GetBytes(directy);
